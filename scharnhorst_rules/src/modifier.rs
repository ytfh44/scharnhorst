...
impl ModifierRegistry {
 ...
 let key = (modifier.target_table.clone(), modifier.target_column.clone());
...
.map(|v| v.as_slice())
 ...
 let (adds, muls, overrides): (Vec<_>, Vec<_>, Vec<_>) = mods
.iter()
.map(|m| m.op)
.zip(mods.iter())
.fold(
 ...
 let after_add = adds
.iter()
.try_fold(base, |acc, m| m.apply(acc))?;

 let after_mul = muls
.iter()
.try_fold(after_add, |acc, m| m.apply(acc))?;
...
mod tests {
 ...
 #[test]
 fn modifier_new() {
 let m = Modifier::new("t", "c", ModifierOp::Add, FixedPoint::from_i64(5, 0).unwrap());
 ...
 reg.register(Modifier::new("t1", "c1", ModifierOp::Add, FixedPoint::from_i64(1, 0).unwrap()));
...
.map_err(|e| RuleError::Generic(e.to_string()))?;
 let back: Modifier = serde_json::from_str(&json)
.map_err(|e| RuleError::Generic(e.to_string()))?;
 assert_eq!(m, back);
 Ok(())
 }
...
.map_err(|e| RuleError::Generic(e.to_string()))?;
 let back: ModifierOp = serde_json::from_str(&json)
.map_err(|e| RuleError::Generic(e.to_string()))?;
 assert_eq!(op, back);
 }
 Ok(())
 ...
 let mut reg = ModifierRegistry::new();
 let m1 = Modifier::new("t", "c", ModifierOp::Add, FixedPoint::from_i64(1, 2).unwrap());
 let m2 = Modifier::new("t", "c", ModifierOp::Mul, FixedPoint::new(150, 2));
 let m3 = Modifier::new("t", "c", ModifierOp::Override, FixedPoint::from_i64(42, 2).unwrap());
