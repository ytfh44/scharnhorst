...
mod tests {
    ...
    fn expr_compare_all_six_operators() {
        ...
        for (op, op_str) in &expected_ops {
            ...
            if let Ok(json) = json_result {
                assert_eq!(
                    json["Compare"]["op"], *op_str,
                    "op serialization mismatch for {:?}",
                    op
                );
    ...
    fn expr_if_then_else_nested_roundtrip() {
        ...
        assert!(
            json_result.is_ok(),
            "nested If serialization should succeed"
        );
        if let Ok(json_str) = json_result {
            let back_result: Result<Expr, _> = serde_json::from_str(&json_str);
            assert!(
                back_result.is_ok(),
                "nested If deserialization should succeed"
            );
            if let Ok(back) = back_result {
    ...
    fn expr_call_named_args_serialization() {
        ...
        args.insert(
            "y".to_string(),
            Expr::constant(FixedPoint::from_i64(20, 0).unwrap()),
        );
    ...
    fn expr_list_mixed_types_preserve_order() {
        ...
        if let Ok(json) = json_result {
            ...
            if let Some(a) = arr {
                ...
                assert!(
                    a[0]["Const"].is_object(),
                    "first element should be Const (FixedPoint object)"
                );
                assert_eq!(a[1]["Bool"], true, "second element should be Bool(true)");
                assert_eq!(
                    a[2]["String"], "hello",
                    "third element should be String(\"hello\")"
                );
                assert!(
                    a[3]["Const"].is_object(),
                    "fourth element should be Const (FixedPoint object)"
                );
            }
        }
    ...
    fn expr_map_key_value_preserve_structure() {
        ...
        entries.insert(
            "health".to_string(),
            Expr::constant(FixedPoint::from_i64(100, 0).unwrap()),
        );
        ...
        if let Ok(json) = json_result {
            ...
            if let Some(obj) = map_obj {
                ...
                assert!(
                    obj["health"]["Const"].is_object(),
                    "health value should be Const (FixedPoint object)"
                );
                assert_eq!(
                    obj["name"]["String"], "orc",
                    "name value should be String(\"orc\")"
                );
            }
        }
    ...
    fn dc1_journal_diff_effect_if_then_else() {
        ...
        if let Ok(json) = json_result {
            ...
            assert_eq!(json["If"]["then_branch"]["Arithmetic"]["op"], "Mul");
            let else_const = json["If"]["else_branch"]["Const"].as_object();
            assert!(
                else_const.is_some(),
                "else_branch Const should be an object"
            );
            if let Some(obj) = else_const {
                assert!(
                    obj.contains_key("raw"),
                    "FixedPoint Const should have 'raw'"
                );
            }
        }
    ...
    fn dc1_multi_modifier_chain_separate_nodes() {
        ...
        if let Ok(json) = json_result {
            ...
            // Inner (lhs) is Add
            assert_eq!(json["Arithmetic"]["lhs"]["Arithmetic"]["op"], "Add");
    ...
    fn expr_roundtrip_all_variants() {
        ...
        // 1. Const
        assert!(roundtrip(&Expr::Const(
            FixedPoint::from_i64(42, 0).unwrap()
        )));
        // 2. Bool
    ...
    fn expr_deeply_nested_depth5_serialization() {
        ...
        assert!(
            json_result.is_ok(),
            "deeply nested serialization should succeed"
        );
        if let Ok(json_str) = json_result {
            let back_result: Result<Expr, _> = serde_json::from_str(&json_str);
            assert!(
                back_result.is_ok(),
                "deeply nested deserialization should succeed"
            );
            if let Ok(back) = back_result {
                assert_eq!(
                    outer_not, back,
                    "deeply nested round-trip should preserve structure"
                );
            }
        }
        ...
        if let Ok(json) = json_val_result {
            ...
            assert!(
                leaf.is_object(),
                "deepest leaf should be Const (a FixedPoint object)"
            );
        }
    }
