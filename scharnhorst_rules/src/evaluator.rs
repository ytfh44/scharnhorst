...
impl Evaluator {
 ...
 match (&args[0], &args[1]) {
 (EvalValue::FixedPoint(a), EvalValue::FixedPoint(b)) => Ok(
 EvalValue::FixedPoint((*a + *b).map_err(RuleError::from)?),
 ),
...
.ok_or_else(|| RuleError::UnknownVariable(var.name.clone()))
 ...
 pub fn submit_diff(&self, diff: Diff) -> RuleResult<()> {
 let mut journal = self
.journal
.write()
.map_err(|_| RuleError::JournalWrapper("poisoned lock".to_owned()))?;
 journal.submit_diff(diff).map_err(RuleError::from)
 }

 /// Submit a command envelope to the journal.
 pub fn submit_command(&self, envelope: CommandEnvelope) -> RuleResult<()> {
 let mut journal = self
.journal
.write()
.map_err(|_| RuleError::JournalWrapper("poisoned lock".to_owned()))?;
 journal.submit_command(envelope).map_err(RuleError::from)
 }
 ...
 match effect {
 Effect::UpdateColumn { table, column, value } => {
...
.unwrap_or(0);
 ...
 EvalValue::FixedPoint(
 self.modifiers
.aggregate(&path.table, &path.column, fp)?,
 )
 }
 ColumnKind::Float64 => {
 let val = lookup
...
.unwrap_or_default();
 ...
 items
.iter()
.try_fold(EvalValue::Bool(true), |acc, expr| {
 let val = self.evaluate(expr)?;
 match (acc, val) {
 ...
 items
.iter()
.try_fold(EvalValue::Bool(false), |acc, expr| {
 let val = self.evaluate(expr)?;
 match (acc, val) {
 (EvalValue::Bool(a), EvalValue::Bool(b)) => Ok(EvalValue::Bool(a || b)),
 _ => Err(RuleError::TypeMismatch {
...
.builtins
.get(func)
.ok_or_else(|| RuleError::UnsupportedOperation(format!("unknown function: {}", func)))?;
...
.map_err(RuleError::from)?
.ok_or_else(|| {
 RuleError::Scope("target row has null PK".to_owned())
 })?;
 if pk_val == fk_val {
 return Ok(row_id);
 }
...
.map_err(RuleError::from)?
.ok_or_else(|| {
 RuleError::Scope("current row PK is null".to_owned())
 })?;
...
.map_err(RuleError::from)?
.ok_or_else(|| {
 RuleError::Scope("row has null FK".to_owned())
 })?;
...
.map_err(|_| RuleError::QueryEngine("poisoned lock".to_owned()))?;
 let view = qe
.read_single_table(self.cache.tick(), &table)
.map_err(RuleError::from)?;
...
mod tests {
 ...
 let journal = std::sync::Arc::new(std::sync::RwLock::new(
 Journal::new(ArrowStore::new()),
 ));
 let query_engine = std::sync::Arc::new(std::sync::RwLock::new(
 ...
 assert!(result.is_ok());
 assert_eq!(result.unwrap(), EvalValue::FixedPoint(FixedPoint::from_i64(42, 0).unwrap()));
 }
 ...
 let a_scale2 = FixedPoint::from_i64(10, 2).unwrap(); // raw=1000
 let b_scale2 = FixedPoint::from_i64(10, 2).unwrap(); // raw=1000

 // Same scale: equal
 assert!(compare_fixed_point(CompareOp::Eq, a_scale2, b_scale2));
 assert!(!compare_fixed_point(CompareOp::Ne, a_scale2, b_scale2));

 // Different scales: FixedPoint identity compares both fields
 let c_scale0 = FixedPoint::from_i64(10, 0).unwrap(); // raw=10
 ...
 // FixedPoint
 ev.bind_var("fp", EvalValue::FixedPoint(FixedPoint::from_i64(42, 2).unwrap()));
 ...
 *ev.resolve_var(&VarRef::new("rid")).unwrap(),
 ...
 let mut ev = make_evaluator();
 ev.bind_var("x", EvalValue::FixedPoint(FixedPoint::from_i64(1, 0).unwrap()));
 assert_eq!(
 *ev.resolve_var(&VarRef::new("x")).unwrap(),
 ...
 // Overwrite
 ev.bind_var("x", EvalValue::FixedPoint(FixedPoint::from_i64(99, 0).unwrap()));
 assert_eq!(
 *ev.resolve_var(&VarRef::new("x")).unwrap(),
 ...
 assert_eq!(*val, EvalValue::FixedPoint(FixedPoint::from_i64(500, 2).unwrap()));
 }
 ...
 *ev.resolve_var(&VarRef::new("y")).unwrap(),
 ...
 assert_eq!(*val, EvalValue::FixedPoint(FixedPoint::from_i64(50, 1).unwrap()));
 }
 ...
 /// Note: the current implementation uses try_fold and evaluates all items,
 /// so a later type-mismatch item would still cause an error.
 #[test]
 ...
 let result = ev.evaluate(&expr).unwrap();
 // try_fold starts with Bool(true); empty iter returns the identity
 assert_eq!(result, EvalValue::Bool(true));
 }
 ...
 let result = ev.evaluate(&expr).unwrap();
 // try_fold starts with Bool(false); empty iter returns the identity
 assert_eq!(result, EvalValue::Bool(false));
 }
 ...
 args.insert("x".to_owned(), Expr::constant(FixedPoint::from_i64(2, 2).unwrap()));
 args.insert("y".to_owned(), Expr::constant(FixedPoint::from_i64(3, 2).unwrap()));
 let result = ev.evaluate(&Expr::call("add", args)).unwrap();
 assert_eq!(result, EvalValue::FixedPoint(FixedPoint::from_i64(5, 2).unwrap()));
 ...
 assert_eq!(result, EvalValue::FixedPoint(FixedPoint::from_i64(6, 2).unwrap()));

 // min: min(5.00, 3.00) = 3.00
 let mut args = HashMap::new();
 args.insert("x".to_owned(), Expr::constant(FixedPoint::from_i64(5, 2).unwrap()));
 args.insert("y".to_owned(), Expr::constant(FixedPoint::from_i64(3, 2).unwrap()));
 let result = ev.evaluate(&Expr::call("min", args)).unwrap();
 assert_eq!(result, EvalValue::FixedPoint(FixedPoint::from_i64(3, 2).unwrap()));
