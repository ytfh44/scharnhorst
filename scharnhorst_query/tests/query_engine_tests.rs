...
use scharnhorst_query::{
 BatchColumnReader, ColumnView, DebugWriteJournal, DebugWriteOp,
 InMemoryReadSource, InspectorConsole, QueryEngine, QueryError, QueryResult,
 ReadRequest, ReadResponse, RowCursor, TableReadView, TypedColumnAccess,
 UnifiedReadSource,
};
...
fn make_engine_with_data() -> QueryEngine {
 ...
 engine
.ingest_snapshot(Tick(1), "units", vec![sample_batch()], RowPositionMap::new())
.unwrap();
 engine
}
...
fn typed_column_access_i64() {
 ...
 assert_eq!(TypedColumnAccess::<i64>::get(&view, 0).unwrap(), Some(10i64));
 assert_eq!(TypedColumnAccess::<i64>::get(&view, 1).unwrap(), Some(20i64));
 assert_eq!(TypedColumnAccess::<i64>::get(&view, 2).unwrap(), Some(30i64));
 assert!(matches!(TypedColumnAccess::<i64>::get(&view, 5), Err(QueryError::IndexOutOfBounds(5))));
}
...
.collect();
 ...
 assert_eq!(cursor.get_typed::<String>("name").unwrap(), Some("alpha".to_owned()));
...
fn dc10_table_read_view_with_sample_batch_reads_correctly() {
 ...
 assert_eq!(view.total_rows(), 3,
 "DC-10: sample_batch has 3 rows");
 assert_eq!(view.batch_count(), 1,
 "DC-10: single batch in vec");

 // first_batch_reader works
 let reader = view.first_batch_reader();
 assert!(reader.is_ok(), "DC-10: first_batch_reader must succeed with data");
 let reader = reader.unwrap();
 assert_eq!(reader.row_count(), 3);

 // first_batch_cursor works
 let cursor = view.first_batch_cursor();
 assert!(cursor.is_ok(), "DC-10: first_batch_cursor must succeed with data");
 let cursor = cursor.unwrap();
 assert_eq!(cursor.row_count(), 3);

 // first_batch_columns works
 let cols = view.first_batch_columns();
 assert!(cols.is_ok(), "DC-10: first_batch_columns must succeed with data");
 let cols = cols.unwrap();
 assert!(cols.contains_key("id"), "DC-10: columns must contain 'id'");
 assert!(cols.contains_key("name"), "DC-10: columns must contain 'name'");
 assert!(cols.contains_key("health"), "DC-10: columns must contain 'health'");
 assert!(cols.contains_key("active"), "DC-10: columns must contain 'active'");
}
...
fn dc10_read_response_insert_and_get() {
 ...
 assert!(result.is_ok(), "DC-10: get('units') must return Ok after insert");
...
fn dc10_read_response_len_and_is_empty() {
 ...
 let view = TableReadView::new("units", Tick(1), vec![sample_batch()], spec, None);
 resp.insert(view);
 assert!(!resp.is_empty(), "DC-10: ReadResponse must not be empty after insert");
 assert_eq!(resp.len(), 1, "DC-10: ReadResponse len must be 1 after one insert");
}
...
fn dc10_in_memory_read_source_register_and_read() {
 ...
 assert!(resp.is_ok(), "DC-10: read must succeed for registered table");
...
fn dc10_in_memory_read_source_unregister_and_clear() {
 ...
 assert!(result.is_ok(), "DC-10: unregister existing table must return Ok");
...
fn dc10_unified_read_source_is_the_only_read_interface() {
 ...
 assert_eq!(scheduler_view.total_rows(), 3,
...
fn dc4_sql_unregister_table_removes_from_registered() {
 ...
 let ctx = scharnhorst_query::SqlExecutionContext::new();
 ctx.register_table("units", vec![sample_batch()]).unwrap();
 assert!(ctx.registered_tables().unwrap().contains(&"units".to_string()));
...
fn dc4_debug_write_journal_new_creates_empty() {
 ...
 journal.len(),
 0,
 "DC-4: new journal must have len=0"
 );
 assert!(
 journal.is_empty(),
 "DC-4: new journal must be empty"
 );
 assert!(
 journal.enabled(),
...
fn dc4_debug_write_journal_zero_capacity_allowed() {
 ...
 let ops = journal.ops();
 assert_eq!(ops[0].tick(), Tick(2), "DC-4: only the latest op survives at capacity 0");
}
...
fn dc4_debug_write_op_patch_table_name_and_tick() {
 ...
 op.tick(),
 Tick(3),
 "DC-4: Patch tick() must return Tick(3)"
 );
}
...
fn dc4_debug_write_record_and_ops_roundtrip() {
 ...
 let ops = journal.ops();
 assert_eq!(ops.len(), 1, "DC-4: ops() must return exactly 1 op after one record");
 assert_eq!(
 ops[0].table_name(),
 "units",
...
fn dc4_debug_write_record_multiple_ops_preserves_order() {
 ...
 assert_eq!(ops[0].tick(), Tick(1), "DC-4: first op must be Append at Tick(1)");
 assert_eq!(ops[1].tick(), Tick(2), "DC-4: second op must be Patch at Tick(2)");
 assert_eq!(ops[2].tick(), Tick(3), "DC-4: third op must be Delete at Tick(3)");
}
...
fn dc4_debug_write_ops_for_table_filters_correctly() {
 ...
 assert_eq!(unit_ops[0].tick(), Tick(1), "DC-4: first 'units' op at Tick(1)");
 assert_eq!(unit_ops[1].tick(), Tick(3), "DC-4: second 'units' op at Tick(3)");
...
fn dc4_debug_write_clear_removes_all_ops() {
 ...
 journal.len(),
 0,
 "DC-4: after clear, len() must be 0"
 );
 assert!(
 journal.is_empty(),
...
.unwrap();
 ...
 let batch2 =
 RecordBatch::try_new(schema, vec![id2, name2, health2, active2]).unwrap();
...
fn dc4_ingest_snapshot_updates_latest_tick() {
 ...
 // ingest at Tick(5)
 engine
.ingest_snapshot(Tick(5), "units", vec![sample_batch()], RowPositionMap::new())
.unwrap();
...
.unwrap();
 ...
 let edge = engine
.resolve_relation_edge("province -> actor")
.unwrap();
 assert_eq!(
 edge.from, "province",
 ...
 assert_eq!(
 edge.to, "actor",
 "DC-6: resolved edge must have to='actor'"
 );
}
