...
impl DeterministicRng {
 /// Create a new deterministic RNG for the given system and tick.
    pub fn new(system_id: impl Into<String>, tick: u64) -> Self {
        ...
        let state = Self::seed(&system_id, tick);
        Self {
            state,
            tick,
            system_id,
 ...
 /// Generate the next `u64` in the deterministic sequence.
    pub fn next_u64(&mut self) -> u64 {
        self.state = Self::splitmix64(self.state);
        self.state
    }
 ...
 // Use the upper 53 bits for a precise f64 in [0,1)
 ...
 /// Generate the next `usize` in the range `[0, bound)`.
    ...
    fn seed(system_id: &str, tick: u64) -> u64 {
 ...
 /// SplitMix64 algorithm -- fast, high-quality 64-bit PRNG.
...
mod tests {
    ...
    fn splitmix64_is_deterministic() {
        let a = DeterministicRng::splitmix64(12345);
        let b = DeterministicRng::splitmix64(12345);
        assert_eq!(a, b);
    }
