use scharnhorst_arrow_store::{InitStore, MutationMode};
use scharnhorst_content::{migrated_from_manifest, LoadLifecycle, LoadPhase, ModFingerprint};
use scharnhorst_core::{StateTier, Tick, TierRegistry};
use scharnhorst_journal::CommitRecord;
use scharnhorst_schema::manifest::{MigratedSchemaManifest, SchemaManifest};
use scharnhorst_schema::relation::RelationGraph;
use scharnhorst_schema::SchemaError;

use crate::error::{SaveError, SaveResult};
use crate::migration::MigrationPipeline;
use crate::mod_tolerance::ModToleranceChecker;
use crate::snapshot_persistence::SnapshotPersistence;

/// Six-phase load reconstruction state machine.
pub struct LoadReconstruction {
    lifecycle: LoadLifecycle,
    persistence: SnapshotPersistence,
    tolerance_checker: ModToleranceChecker,
    migration_pipeline: Option<MigrationPipeline>,
    tier_registry: TierRegistry,
}

impl LoadReconstruction {
    pub fn new(persistence: SnapshotPersistence, tolerance_checker: ModToleranceChecker) -> Self {
        Self {
            lifecycle: LoadLifecycle::new(),
            persistence,
            tolerance_checker,
            migration_pipeline: None,
            tier_registry: TierRegistry::new(),
        }
    }

    pub fn with_migration_pipeline(mut self, pipeline: MigrationPipeline) -> Self {
        self.migration_pipeline = Some(pipeline);
        self
    }

    /// Attach a TierRegistry pre-populated with authority table registrations.
    pub fn with_tier_registry(mut self, tier_registry: TierRegistry) -> Self {
        self.tier_registry = tier_registry;
        self
    }

    pub fn lifecycle(&self) -> &LoadLifecycle {
        &self.lifecycle
    }

    /// Skip directly to a given phase for testing or recovery.
    pub fn skip_to_phase(&mut self, phase: LoadPhase) -> SaveResult<()> {
        self.lifecycle
            .start_phase(phase)
            .map_err(SaveError::Content)?;
        Ok(())
    }

    /// Access the TierRegistry for authority-table filtering and state management.
    pub fn tier_registry(&self) -> &TierRegistry {
        &self.tier_registry
    }

    /// Phase 1: Deserialize the latest snapshot header.
    pub fn phase1_snapshot_deserialize(&mut self) -> SaveResult<SchemaManifest> {
        let phase = self.lifecycle.advance()?;
        if phase != LoadPhase::SnapshotDeserialize {
            return Err(SaveError::InvalidPhaseTransition {
                from: phase.name().to_owned(),
                to: LoadPhase::SnapshotDeserialize.name().to_owned(),
            });
        }
        let (tick, _) = self
            .persistence
            .latest_snapshot()?
            .ok_or_else(|| SaveError::SnapshotNotFound("no snapshots available".to_owned()))?;
        let persisted = self.persistence.read_snapshot(tick)?;
        let manifest = persisted.header.schema_manifest;
        self.lifecycle.set_manifest(manifest.clone());
        Ok(manifest)
    }

    /// Phase 2: Compare stored mod fingerprints against available mods.
    pub fn phase2_mod_coordination(
        &mut self,
        stored: &scharnhorst_content::FingerprintRegistry,
        available: &[ModFingerprint],
    ) -> SaveResult<Vec<ModFingerprint>> {
        let phase = self.lifecycle.advance()?;
        if phase != LoadPhase::ModCoordination {
            return Err(SaveError::InvalidPhaseTransition {
                from: phase.name().to_owned(),
                to: LoadPhase::ModCoordination.name().to_owned(),
            });
        }
        let _outcome = self.tolerance_checker.check(stored, available)?;
        let final_mods = self.tolerance_checker.final_mod_list(stored, available)?;
        self.lifecycle.set_final_mods(final_mods.clone());
        Ok(final_mods)
    }

    /// Phase 3: Apply schema migrations to bring manifest up to current version.
    pub fn phase3_schema_migration(
        &mut self,
        manifest: &SchemaManifest,
    ) -> SaveResult<MigratedSchemaManifest> {
        let phase = self.lifecycle.advance()?;
        if phase != LoadPhase::SchemaMigration {
            return Err(SaveError::InvalidPhaseTransition {
                from: phase.name().to_owned(),
                to: LoadPhase::SchemaMigration.name().to_owned(),
            });
        }
        let migrated = match &self.migration_pipeline {
            Some(pipeline) => pipeline.apply(manifest)?,
            None => migrated_from_manifest(manifest, &manifest.schema_version),
        };
        self.lifecycle.set_migrated_manifest(migrated.clone());
        Ok(migrated)
    }

    /// Phase 4: Create Authority tables in the ArrowStore using InitStore.
    ///
    /// Each table from the migrated manifest is created in the store and
    /// registered as `Authority` tier in the TierRegistry.
    /// Does NOT populate data -- that remains the caller's responsibility
    /// after compilation.
    pub fn phase4_content_compilation_entry(&mut self, init_store: &InitStore) -> SaveResult<()> {
        let phase = self.lifecycle.advance()?;
        if phase != LoadPhase::ContentCompilation {
            return Err(SaveError::InvalidPhaseTransition {
                from: phase.name().to_owned(),
                to: LoadPhase::ContentCompilation.name().to_owned(),
            });
        }
        let migrated = self
            .lifecycle
            .migrated_manifest()
            .ok_or_else(|| SaveError::Generic("migrated manifest not set".to_owned()))?;

        let mut graph = RelationGraph::new();
        for edge in migrated.relations() {
            graph.add_edge(edge.clone())?;
        }
        let cycles = graph.detect_cycles();
        if !cycles.is_empty() {
            let cycle_desc: Vec<String> = cycles.iter().map(|c| c.join(" -> ")).collect();
            return Err(SaveError::Schema(SchemaError::CircularRelation(
                cycle_desc.join("; "),
            )));
        }

        for table in migrated.tables() {
            init_store
                .create_table(table, MutationMode::AppendOnly)
                .map_err(SaveError::ArrowStore)?;
            let _ = self.tier_registry.register(
                &table.name,
                StateTier::Authority,
                "save_loader",
                format!("Loaded authority table: {}", table.name),
            );
        }

        Ok(())
    }

    /// Return table names from the migrated manifest that are NOT registered
    /// as Authority. These are candidates for Derived or Ephemeral
    /// initialization after load.
    pub fn non_authority_tables(&self) -> SaveResult<Vec<String>> {
        let migrated = self
            .lifecycle
            .migrated_manifest()
            .ok_or_else(|| SaveError::Generic("migrated manifest not set".to_owned()))?;

        let result: Vec<String> = migrated
            .tables()
            .iter()
            .filter(|t| !self.tier_registry.is_authority(&t.name))
            .map(|t| t.name.clone())
            .collect();
        Ok(result)
    }

    /// Table names that should have Derived state marked as needing rebuild.
    ///
    /// After an Authority-only snapshot load, any loaded table not in the
    /// Authority tier should be treated as Derived and marked dirty for
    /// rebuild at the next tick.
    pub fn derived_tables_need_rebuild(&self) -> SaveResult<Vec<String>> {
        self.non_authority_tables()
    }

    /// Table names that should be initialized as Ephemeral from defaults.
    ///
    /// Ephemeral tables are not persisted and must be re-created with
    /// default (empty) state on every load.
    pub fn ephemeral_tables_needing_init(&self) -> SaveResult<Vec<String>> {
        self.non_authority_tables()
    }

    /// Phase 5 entry point: signal that schema registry should freeze.
    pub fn phase5_schema_freeze(&mut self) -> SaveResult<()> {
        let phase = self.lifecycle.advance()?;
        if phase != LoadPhase::SchemaFreeze {
            return Err(SaveError::InvalidPhaseTransition {
                from: phase.name().to_owned(),
                to: LoadPhase::SchemaFreeze.name().to_owned(),
            });
        }
        self.lifecycle.set_frozen(true);
        Ok(())
    }

    /// Phase 6 entry point: signal simulation start.
    pub fn phase6_simulation_start(&mut self) -> SaveResult<()> {
        let phase = self.lifecycle.advance()?;
        if phase != LoadPhase::SimulationStart {
            return Err(SaveError::InvalidPhaseTransition {
                from: phase.name().to_owned(),
                to: LoadPhase::SimulationStart.name().to_owned(),
            });
        }
        self.lifecycle.finish().map_err(SaveError::Content)?;
        Ok(())
    }

    /// Replay journal diffs on top of a snapshot to reconstruct a later state.
    ///
    /// Accumulates each record's `state_hash` via `wrapping_add`, matching
    /// the per-commit hash produced by [`Journal::apply_diffs_and_hash`].
    /// The initial hash comes from the snapshot at `base_tick`.
    pub fn replay_diffs(&self, base_tick: Tick, records: &[CommitRecord]) -> SaveResult<u64> {
        let snapshot = self.persistence.read_snapshot(base_tick)?;
        let mut current_hash = snapshot.header.state_hash;
        for record in records.iter().filter(|r| r.tick > base_tick) {
            current_hash = current_hash.wrapping_add(record.state_hash);
        }
        Ok(current_hash)
    }

    /// Verify that a computed state hash matches the stored hash.
    pub fn verify_state_hash(computed: u64, expected: u64) -> SaveResult<()> {
        if computed != expected {
            return Err(SaveError::StateHashMismatch {
                expected,
                got: computed,
            });
        }
        Ok(())
    }
}

/// Builder for configuring and running the full six-phase load.
pub struct LoadReconstructionBuilder {
    persistence: Option<SnapshotPersistence>,
    tolerance_checker: Option<ModToleranceChecker>,
    migration_pipeline: Option<MigrationPipeline>,
    tier_registry: Option<TierRegistry>,
}

impl LoadReconstructionBuilder {
    pub fn new() -> Self {
        Self {
            persistence: None,
            tolerance_checker: None,
            migration_pipeline: None,
            tier_registry: None,
        }
    }

    pub fn persistence(mut self, p: SnapshotPersistence) -> Self {
        self.persistence = Some(p);
        self
    }

    pub fn tolerance_checker(mut self, c: ModToleranceChecker) -> Self {
        self.tolerance_checker = Some(c);
        self
    }

    pub fn migration_pipeline(mut self, p: MigrationPipeline) -> Self {
        self.migration_pipeline = Some(p);
        self
    }

    pub fn tier_registry(mut self, tr: TierRegistry) -> Self {
        self.tier_registry = Some(tr);
        self
    }

    pub fn build(self) -> SaveResult<LoadReconstruction> {
        let persistence = self
            .persistence
            .ok_or_else(|| SaveError::Generic("persistence required".to_owned()))?;
        let tolerance_checker = self
            .tolerance_checker
            .ok_or_else(|| SaveError::Generic("tolerance_checker required".to_owned()))?;
        let mut recon = LoadReconstruction::new(persistence, tolerance_checker);
        if let Some(pipeline) = self.migration_pipeline {
            recon = recon.with_migration_pipeline(pipeline);
        }
        if let Some(tr) = self.tier_registry {
            recon = recon.with_tier_registry(tr);
        }
        Ok(recon)
    }
}

impl Default for LoadReconstructionBuilder {
    fn default() -> Self {
        Self::new()
    }
}

#[cfg(test)]
mod tests {
    use super::*;
    use crate::snapshot_persistence::{PersistedSnapshot, SnapshotHeader, SnapshotPersistence};
    use scharnhorst_content::{migrated_target_version, SchemaManifest};
    use std::path::PathBuf;

    fn temp_dir() -> PathBuf {
    use std::sync::atomic::{AtomicUsize, Ordering};
    static COUNTER: AtomicUsize = AtomicUsize::new(0);
    let id = COUNTER.fetch_add(1, Ordering::SeqCst);
    std::env::temp_dir().join(format!("sch_load_test_{}_{}", std::process::id(), id))
}

fn cleanup(dir: &std::path::Path) {
    let _ = std::fs::remove_dir_all(dir);
}

    fn dummy_manifest() -> SchemaManifest {
        SchemaManifest::new("1.0.0")
    }

    #[test]
    fn builder_requires_persistence() {
        let result = LoadReconstructionBuilder::new()
            .tolerance_checker(ModToleranceChecker::default())
            .build();
        assert!(result.is_err());
    }

    #[test]
    fn phase3_no_migration_needed() -> SaveResult<()> {
    let dir = temp_dir();
    cleanup(&dir);
    std::fs::create_dir_all(&dir).map_err(|e| SaveError::Io(e.to_string()))?;
    let persistence = SnapshotPersistence::new(&dir);
    let mut recon = LoadReconstruction::new(persistence, ModToleranceChecker::default());
    recon.skip_to_phase(LoadPhase::ModCoordination)?;
    let manifest = dummy_manifest();
    let migrated = recon.phase3_schema_migration(&manifest)?;
    assert_eq!(migrated_target_version(&migrated), "1.0.0");
    cleanup(&dir);
    Ok(())
}

    #[test]
    fn replay_empty_diffs() -> SaveResult<()> {
    let dir = temp_dir();
    cleanup(&dir);
    std::fs::create_dir_all(&dir).map_err(|e| SaveError::Io(e.to_string()))?;
    let persistence = SnapshotPersistence::new(&dir);
    let manifest = SchemaManifest::new("1.0.0");
    let header = SnapshotHeader::new(manifest, Tick(0), 0);
    let snapshot = PersistedSnapshot::new(header);
    persistence.write_snapshot(&snapshot)?;
    let recon = LoadReconstruction::new(persistence, ModToleranceChecker::default());
    let hash = recon.replay_diffs(Tick(0), &[])?;
    assert_eq!(hash, 0);
    cleanup(&dir);
    Ok(())
}

    #[test]
    fn verify_matching_hash() -> SaveResult<()> {
        LoadReconstruction::verify_state_hash(42, 42)?;
        Ok(())
    }

    #[test]
    fn verify_mismatched_hash() {
        let result = LoadReconstruction::verify_state_hash(42, 43);
        assert!(matches!(result, Err(SaveError::StateHashMismatch { .. })));
    }
}
