...
### (Compatibility) Requirement: Relation Graph Maintenance (↔ content-loader, ↔ query-engine)
...
- **rule-ir**: Indirectly via `query-engine.lookup_row()` — direct access forbidden (see `query-engine` DC-10)
...
### (Compatibility) Requirement: Mod Fingerprint Tracking (↔ content-loader, ↔ save-system)
...
- **THEN** the content-loader flags a mismatch; the `save-system` applies graceful degradation (see `save-system` Compatibility Requirement: Mod-Aware Load Tolerance).

### (Compatibility) Requirement: Cold-Start Registration Contract (↔ content-loader, ↔ save-system)
