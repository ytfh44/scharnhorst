...
### I-ST-TIER-TAXONOMY
Every byte of runtime world state is classified as exactly one of Authority, Derived, or Ephemeral. Authority state lives in `ArrowStore` and participates in journal diffs, save snapshots, and deterministic state hashes. Derived state is rebuildable from Authority state and NOT persisted as proof data. Ephemeral state is frame-local, UI-local, or transient and NOT stored in `ArrowStore`.

### I-ST-AUTHORITY-HASH
The deterministic state hash computed at each tick boundary depends ONLY on Authority state and committed diffs. Derived and Ephemeral state are excluded from the hash computation. Two replays of identical input commands produce identical Authority hashes regardless of non-deterministic Derived rebuild timing or Ephemeral animation differences.

### I-ST-FIXED-POINT-ORD
`FixedPoint` `Ord` compares mathematical values, not `(raw, scale)` tuples. `FixedPoint(100, 2)` (1.00) > `FixedPoint(200, 3)` (0.200). `FixedPoint(1, 0)` (1.0) == `FixedPoint(10, 1)` (1.0). `FixedPoint::rescale` uses banker's rounding (round half to even) to minimize cumulative precision loss.

### I-ST-TIER-DEDUP
`TierRegistry::register()` SHALL reject duplicate table-name registration. Silent overwrite SHALL NOT occur. The existing entry SHALL be preserved and a descriptive error returned identifying the table name and existing tier.