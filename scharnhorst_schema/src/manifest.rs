...
mod tests {
 ...
 // ---- SchemaManifest ----
    ...
    fn schema_manifest_construction() {
        ...
        assert_eq!(manifest.metadata.get("engine_version"), Some(&"0.1.0".to_string()));
    }
    ...
    fn schema_manifest_json_roundtrip() {
        ...
        assert_eq!(restored.mod_fingerprints.len(), manifest.mod_fingerprints.len());
    }
 ...
 // ---- MigratedSchemaManifest ----
    ...
    fn migrated_schema_manifest_construction() {
        let manifest = SchemaManifest::new("1.1.0")
            .with_table(make_test_table("actor_state"));

        let migrated = MigratedSchemaManifest::new(manifest, "1.0.0")
            .record_migration("v1_0_to_v1_1");
